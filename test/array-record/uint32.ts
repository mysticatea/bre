...
describe("defineArrayRecord:", () => {
    describe("ArrayRecord with 'uint32' and 2", () => {
        const TestRecord = defineArrayRecord("uint32", 2)
        let buffer: Buffer
        ...
        it("should write the 1st..4th bytes", () => {
            record[0] = 4275878552
            record[1] = 1985229328
            assert.strictEqual(record[0], 4275878552)
        ...
        it("should be ok if it wrote 4294967295", () => {
            for (const k of record.keys()) {
                record[k] = 4294967295
                assert.strictEqual(record[k], 4294967295)
            }
        })
        ...
        it("should throw if it wrote 4294967296 (out of range)", () => {
            for (const k of record.keys()) {
                assert.strictEqual(
                    ...
                    `AssertionError: '${k}' should be within '0..4294967295', but got 4294967296.`,
                )
            }
        })
        ...
        it("should throw if it wrote -1 (out of range)", () => {
            for (const k of record.keys()) {
                assert.strictEqual(
                    ...
                    `AssertionError: '${k}' should be within '0..4294967295', but got -1.`,
                )
            }
        })
