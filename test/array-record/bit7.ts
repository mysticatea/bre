...
describe("defineArrayRecord:", () => {
    describe("ArrayRecord with 'bit7' and 2", () => {
        const TestRecord = defineArrayRecord("bit7", 2)
        let buffer: Buffer
        ...
        it("should read the 1st..2nd bytes", () => {
            ...
            assert.strictEqual(record[1], 64)
        ...
        it("should write the 1st..2nd bytes", () => {
            record[0] = 67
            record[1] = 89
            assert.strictEqual(record[0], 67)
            assert.strictEqual(record[1], 89)
            assert.strictEqual(buffer[0], 0x87)
            assert.strictEqual(buffer[1], 0x66)
            assert.strictEqual(buffer[2], 0x03)
            assert.strictEqual(buffer[3], 0x04)
            assert.strictEqual(buffer[4], 0x05)
        })
        ...
        it("should have bitLength 14", () => {
            assert.strictEqual(TestRecord.bitLength, 14)
        })
        ...
        it("should have values [0,64]", () => {
            assert.deepStrictEqual(Array.from(record.values()), [0, 64])
        })

        it("should have entries [[0,0],[1,64]]", () => {
        ...
        it("should throw if it wrote 128 (out of range)", () => {
            for (const k of record.keys()) {
                assert.strictEqual(
                    ...
                    `AssertionError: '${k}' should be within '0..127', but got 128.`,
                )
            }
        })
        ...
        it("should throw if it wrote -1 (out of range)", () => {
            for (const k of record.keys()) {
                assert.strictEqual(
                    ...
                    `AssertionError: '${k}' should be within '0..127', but got -1.`,
                )
            }
        })
