...
describe("defineArrayRecord:", () => {
    describe("Record with 'int16' and 2", () => {
        const Record = defineArrayRecord("int16", 2)
        let buffer = null
        let record = null
        ...
        it("should throw if it wrote -32769 (out of range)", () => {
            for (const k of record.keys()) {
                assert(thrownMessage(() => {
                    record[k] = -32769
                }) === `'${k}' should be within '-32768..32767', but got -32769.`)
            }
        })
