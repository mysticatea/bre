...
describe("defineObjectRecord with IconvLiteTextEncoder:", () => {
    describe("ObjectRecord with 1 '{ encoding: \"utf8\", byteLength: 15 }' field", () => {
        setTextEncoder(IconvLiteTextEncoder)
        const TestRecord = defineObjectRecord("String15_1", {
    ...
    describe("ObjectRecord with 1 '{ encoding: \"shift_jis\", byteLength: 15 }' field", () => {
        setTextEncoder(IconvLiteTextEncoder)
        const TestRecord = defineObjectRecord("ShiftJIS15_1", {
            a: { encoding: "shift_jis", byteLength: 15 },
        })
        let buffer: Buffer
        let record: RecordOf<typeof TestRecord>
        ...
        it("should be possible to address Japanese", () => {
            record.a = "こんにちは"
            assert.strictEqual(record.a, "こんにちは")
            assert.strictEqual(buffer[0], 0x82)
            assert.strictEqual(buffer[1], 0xb1)
            assert.strictEqual(buffer[2], 0x82)
            assert.strictEqual(buffer[3], 0xf1)
            assert.strictEqual(buffer[4], 0x82)
            assert.strictEqual(buffer[5], 0xc9)
            assert.strictEqual(buffer[6], 0x82)
            assert.strictEqual(buffer[7], 0xbf)
            assert.strictEqual(buffer[8], 0x82)
            assert.strictEqual(buffer[9], 0xcd)
            assert.strictEqual(buffer[10], 0x00)
            assert.strictEqual(buffer[11], 0x00)
            assert.strictEqual(buffer[12], 0x00)
            assert.strictEqual(buffer[13], 0x00)
            assert.strictEqual(buffer[14], 0x00)
            assert.strictEqual(buffer[15], 0x20)
        })
